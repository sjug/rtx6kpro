...
# DS4 v19 (dev/gilded-gnosis + b12x master) two-node TP2 launcher for the
# DGX Spark pair (rusty + toby).
#
# Same launch contract as spark/v18/run-ds4-v18-tp2-node.sh: the image ships
# /usr/local/bin/serve-ds4-flash.sh (env-only interface, now from
# local-inference-lab/vllm dev/gilded-gnosis @ 371085e9e). New on this
# lineage: the dspark capacity/SPS-curve/dynamic-draft-depth knobs, all
# consumed by the in-image helper under its own DSPARK_* names.
#
...
IMAGE=${IMAGE:-localhost/voipmonitor/vllm:gilded-gnosis-v19-dspark-spark-sm121-vllm371085e-b12xc7dc733-fice40d25-cu132-20260719}
NAME=${NAME:-ds4-v19-tp2}
PORT=${PORT:-8000}

HF_CACHE=${HF_CACHE:-$HOME/.cache/huggingface}
CACHE=${CACHE:-$HOME/.cache/vllm-ds4-v19}
