...
# Test the supported arithmetic-striding work source against r18p's default
# materialized atomic queue. Both consume the same fully published work domain;
# persistent_grid removes per-task atomic claims but can expose load imbalance.
...
exec > >(tee -a "${ROOT}/work-source-aba.log") 2>&1
...
launch_node() {
  ...
  if [[ "${policy}" == persistent ]]; then
    policy_env='-e B12X_DYNAMIC_WORK_SOURCE=persistent_grid'
  fi
...
validate_live() {
  ...
  for host in "${HEAD_HOST}" "${WORKER_HOST}"; do
    ...
    if [[ "${policy}" == persistent ]]; then
      grep -Fxq 'B12X_DYNAMIC_WORK_SOURCE=persistent_grid' <<<"${env_dump}"
    else
      if grep -q '^B12X_DYNAMIC_WORK_SOURCE=' <<<"${env_dump}"; then
        printf '%s default arm unexpectedly carries a work-source override\n' "${host}" >&2
        return 1
      fi
    fi
  done
...
start_policy() {
  ...
  stop_pair
  needs_restore=1
  printf '[%s] launching A8 work-source policy=%s worker then head\n' "$(timestamp)" "${policy}"
...
restore_default() {
  ...
  printf '[%s] restoring stock r18p A8 work-source behavior\n' "$(timestamp)"
...
run_sweep() {
  ...
  seed="ds4-a8-work-source-v1-sweep${sweep}"
...
measure_arm() {
  ...
  if [[ "${policy}" == persistent ]]; then
    curl -fsS --max-time 120 "http://${HEAD_HOST}:${PORT}/v1/chat/completions" \
      ...
      -d '{"model":"DeepSeek-V4-Flash-0731","messages":[{"role":"user","content":"Return only the integer answer: 41 + 27"}],"temperature":0,"max_tokens":32}' \
      >"${ROOT}/arithmetic-smoke.json"
    jq -e '.choices[0].finish_reason != null and (.choices[0].message.content | contains("68"))' \
...
printf '[%s] beginning queue/persistent/queue work-source comparison\n' "$(timestamp)"
measure_arm default-opening default
measure_arm persistent-candidate persistent
measure_arm default-closing default
needs_restore=0
printf '[%s] work-source A/B/A complete; final staging state is stock r18p A8\n' "$(timestamp)"
