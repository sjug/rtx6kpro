#!/usr/bin/env python3
"""Compare arithmetic striding with bracketed materialized-queue controls."""
...
def main() -> None:
    ...
    persistent = median_paths(
        [
            ROOT / "results" / f"persistent-candidate-sweep{sweep}.json"
            for sweep in (1, 2)
        ]
    )

    print("arm concurrency steps_geo tokens_geo accept_len prefill64k")
    for label, arm in (("queue", default), ("persistent", persistent)):
        for concurrency in (1, 4):
            print(
    ...
    print("\npersistent versus queue")
    print(f"64K prefill: {pct(persistent['prefill64k'], default['prefill64k']):+.2f}%")
    for concurrency in (1, 4):
        print(
            f"c{concurrency}: "
            f"steps={pct(persistent[concurrency]['steps'], default[concurrency]['steps']):+.2f}% "
            f"tokens={pct(persistent[concurrency]['tokens'], default[concurrency]['tokens']):+.2f}% "
            f"accept={pct(persistent[concurrency]['accept'], default[concurrency]['accept']):+.2f}%"
        )
        for context in CONTEXTS:
            print(
                f"  {context}: steps="
                f"{pct(persistent[concurrency]['by_context'][context], default[concurrency]['by_context'][context]):+.2f}%"
            )
