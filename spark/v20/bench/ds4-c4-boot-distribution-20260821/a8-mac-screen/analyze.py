#!/usr/bin/env python3
"""Compare prepared-W4A8 MAC caps with bracketed default controls."""
...
def main() -> None:
    ...
    candidates = {
        cap: median_paths([ROOT / "results" / f"mac{cap}-candidate-sweep1.json"])
        for cap in (24, 32, 40)
    }

    print("arm concurrency steps_geo tokens_geo accept_len prefill64k")
    for label, arm in (("default", default), *candidates.items()):
        for concurrency in (1, 4):
            print(
    ...
    for cap, candidate in candidates.items():
        print(f"\nMAC {cap} versus default")
        print(
            f"64K prefill: {pct(candidate['prefill64k'], default['prefill64k']):+.2f}%"
        )
        for concurrency in (1, 4):
            print(
                f"c{concurrency}: "
                f"steps={pct(candidate[concurrency]['steps'], default[concurrency]['steps']):+.2f}% "
                f"tokens={pct(candidate[concurrency]['tokens'], default[concurrency]['tokens']):+.2f}% "
                f"accept={pct(candidate[concurrency]['accept'], default[concurrency]['accept']):+.2f}%"
            )
            for context in CONTEXTS:
                print(
                    f"  {context}: steps="
                    f"{pct(candidate[concurrency]['by_context'][context], default[concurrency]['by_context'][context]):+.2f}%"
                )
