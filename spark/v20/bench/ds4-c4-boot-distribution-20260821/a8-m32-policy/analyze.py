#!/usr/bin/env python3
"""Compare forced-M32 W4A8 with bracketed default-policy A8 controls."""
...
def main() -> None:
    ...
    m32 = median_paths(
        [ROOT / "results" / f"m32-candidate-sweep{sweep}.json" for sweep in (1, 2)]
    )

    print("arm concurrency steps_geo tokens_geo accept_len prefill64k")
    for label, arm in (("default", default), ("m32", m32)):
        for concurrency in (1, 4):
            print(
    ...
    print("\nM32 versus default")
    print(f"64K prefill: {pct(m32['prefill64k'], default['prefill64k']):+.2f}%")
    for concurrency in (1, 4):
        print(
            f"c{concurrency}: "
            f"steps={pct(m32[concurrency]['steps'], default[concurrency]['steps']):+.2f}% "
            f"tokens={pct(m32[concurrency]['tokens'], default[concurrency]['tokens']):+.2f}% "
            f"accept={pct(m32[concurrency]['accept'], default[concurrency]['accept']):+.2f}%"
        )
        for context in CONTEXTS:
            print(
                f"  {context}: steps="
                f"{pct(m32[concurrency]['by_context'][context], default[concurrency]['by_context'][context]):+.2f}%"
            )
