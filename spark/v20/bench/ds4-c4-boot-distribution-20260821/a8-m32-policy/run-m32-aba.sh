...
# Test whether r18p's M16-to-M32 W4A8 crossover is too conservative for the
# DS4 TP2 K5 verify shapes. The A/B/A sequence uses independent engine starts;
# every arm stays on the exact r18p A8 image and differs only by the documented
# B12X dynamic-tile override.
...
exec > >(tee -a "${ROOT}/m32-aba.log") 2>&1
...
launch_node() {
  ...
  if [[ "${policy}" == m32 ]]; then
    policy_env='-e B12X_DYNAMIC_TILE_MN=32x128'
  fi
...
validate_live() {
  ...
  for host in "${HEAD_HOST}" "${WORKER_HOST}"; do
    ...
    if [[ "${policy}" == m32 ]]; then
      grep -Fxq 'B12X_DYNAMIC_TILE_MN=32x128' <<<"${env_dump}"
    elif grep -q '^B12X_DYNAMIC_TILE_MN=' <<<"${env_dump}"; then
      printf '%s default A8 unexpectedly carries a tile override\n' "${host}" >&2
      return 1
    fi
  done
}
...
start_policy() {
  ...
  stop_pair
  needs_restore=1
  printf '[%s] launching A8 policy=%s worker then head\n' "$(timestamp)" "${policy}"
...
run_sweep() {
  ...
  seed="ds4-a8-m32-policy-v1-sweep${sweep}"
...
printf '[%s] beginning default/M32/default A8 policy comparison\n' "$(timestamp)"
measure_arm default-opening default
measure_arm m32-candidate m32
measure_arm default-closing default
needs_restore=0
printf '[%s] M32 A/B/A complete; final staging state is stock r18p A8\n' "$(timestamp)"
