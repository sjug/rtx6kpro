...
IMAGE=${IMAGE:-localhost/voipmonitor/vllm:gilded-gnosis-v18p4-grid48-spark-sm121-vllmdf7a0b7-b12x6b10833-fi801d57a-cu132-20260718}
NAME=${NAME:-glm52-v18p4-grid48-tp4}
PORT=${PORT:-8000}
NNODES=${NNODES:-4}
...
NF3_MAPPED_GRID=${NF3_MAPPED_GRID:-${NF3_GRID188:-1}}
...
podman run -d \
  ...
  -e VLLM_NF3_MAPPED_GRID_DECODE="$NF3_MAPPED_GRID" \
  -e VLLM_NF3_GRID188_DECODE="$NF3_MAPPED_GRID" \
...
echo "$NAME rank=$NODE_RANK/$NNODES master=$MASTER_ADDR host_ip=$VLLM_HOST_IP tp=$TP dcp=$DCP mtp=$MTP mapped_grid=$NF3_MAPPED_GRID port=$PORT seqs=$MAX_NUM_SEQS graph=$GRAPH len=$MAX_MODEL_LEN gpu_mem=$GPU_MEM gid=${NCCL_IB_GID_INDEX:-auto}"
