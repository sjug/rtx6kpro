#!/usr/bin/env bash
# Overnight v16 accuracy sweep v2 — Phase 0 + Phase 1.
#
# v2 deltas from v1 (after the 23:42 ConnectTimeout flap):
#   - IP-literal HOST (http://192.168.2.45:8000) removes name resolution.
#   - bench(): up to 3 attempts, 240s backoff; retries on nonzero rc with a
#     live server AND on >1/3 request-error fraction inside an "ok" run.
#   - Cell 1 (lucifer) skips launch if already healthy; its battery restarts
#     from the top since v1's hotel run had 22/30 request errors.
set -uo pipefail
...
# rc: 0 ok | 1 gave up, server alive | 2 server dead (abandon cell)
bench() {
  local label=$1 model=$2 profile=$3 cc=$4 tmo=$5; shift 5
  local attempt rc f summary errs att
  for attempt in 1 2 3; do
    step "bench start: $label (profile=$profile cc=$cc attempt=$attempt)"
...
step "overnight sweep v2 begins (host=$HOST_URL)"
...
step "PHASE 0 begin (lucifer-cutlass dspark, seqs=8) [v2: hotel rerun — v1 run had 22/30 errors]"
bench v16-lucifer-hotel   "$DSPARK_MODEL" hotel-lights 8 2700; rc=$?
if (( rc != 2 )); then bench v16-lucifer-estonia "$DSPARK_MODEL" estonia 4 7200; rc=$?; fi
if (( rc != 2 )); then bench v16-lucifer-gsm8k-slice "$DSPARK_MODEL" gsm8k 8 3600 --profile-runs 25 --max-tokens 16384; rc=$?; fi
step "PHASE 0 complete"

step "PHASE 1 begin"
if (( rc != 2 )); then bench v16-lucifer-gsm8k "$DSPARK_MODEL" gsm8k 8 14400 --max-tokens 16384; rc=$?; fi
...
if launch mtp0 b12x-a16; then
  bench v16-mtp0-hotel   "$FLASH_MODEL" hotel-lights 8 2700; rc=$?
  if (( rc != 2 )); then bench v16-mtp0-gsm8k   "$FLASH_MODEL" gsm8k 8 14400 --max-tokens 16384; rc=$?; fi
...
if launch dspark b12x-a16; then
  bench v16-b12xds-hotel   "$DSPARK_MODEL" hotel-lights 8 2700; rc=$?
  if (( rc != 2 )); then bench v16-b12xds-gsm8k   "$DSPARK_MODEL" gsm8k 8 14400 --max-tokens 16384; rc=$?; fi
  if (( rc != 2 )); then bench v16-b12xds-estonia "$DSPARK_MODEL" estonia 4 7200; rc=$?; fi
  if (( rc != 2 )); then bench v16-b12xds-lavd    "$DSPARK_MODEL" lavd-test 8 3600; rc=$?; fi
else
...
if launch mtp2 b12x-a16; then
  bench v16-mtp2-hotel "$FLASH_MODEL" hotel-lights 8 2700; rc=$?
  if (( rc != 2 )); then bench v16-mtp2-gsm8k "$FLASH_MODEL" gsm8k 8 14400 --max-tokens 16384; rc=$?; fi
  if (( rc != 2 )); then bench v16-mtp2-lavd  "$FLASH_MODEL" lavd-test 8 3600; rc=$?; fi
else
