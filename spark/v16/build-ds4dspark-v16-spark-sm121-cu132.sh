...
# DS4/DSpark + GLM unified v16 (Fathomless Firmament) image for DGX Spark
# (GB10, SM121, aarch64).
#
# Same source pins as the canonical x86_64 v16 recipe
# (blackwell-llm-docker/build-fathomless-firmament-v16-cu132.sh @ d104659,
# image voipmonitor/vllm:fathomless-firmament-v16-vllm8f86f42-b12xfe06f49-fi801d57a-cu132-20260714,
# documented in models/ds4dspark-v10.md), retargeted at SM121 via branch
# spark/sm121-arm64-v16 (upstream d104659 merged into the v9/v10 SM121 port).
#
# | Component     | Ref |
# |---------------|-----|
# | vLLM          | codex/fathomless-firmament-v16-unified-20260712 @ 8f86f42 |
# | B12X          | voipmonitor/b12x codex/fathomless-firmament-v16-integration-20260714 @ fe06f49 |
# | FlashInfer    | voipmonitor/flashinfer codex/sm120-dspark-stack-20260711 @ 801d57a |
# | DeepGEMM      | a6b593d (unchanged from v10; SM121 MQA-logits patch still applies) |
# | NCCL          | nccl-canonical canonical/cu132-nccl2304-amd-noxml @ dfab7c1 |
# | InstantTensor | 85e7c5f (now the default runtime loader, BUFFERED) |
# | CUTLASS       | d80a4e5 |
#
# Spark deltas from the canonical recipe:
# - podman instead of docker (CONTAINER_ENGINE + post-build checks below).
# - SM121 arch args (12.1a / 121a).
# - MAX_JOBS=12 for the 20-core GB10.
# - Reuses the v9-era spark-sm121 base images: pin-identical CUDA 13.2.1,
#   torch 2.12.0+cu132, and the SAME canonical NCCL dfab7c1 (verified —
#   the nccl-canonical ref/commit did not change between our base build
#   and d104659).
# - DEEPGEMM_PATCH_FILE for the SM121 MQA-logits arch-number fix.
# - PyNvVideoCodec 2.0.4 / nccl4py 0.3.1 aarch64 wheels verified present.
#
...
export IMAGE="${IMAGE:-voipmonitor/vllm:fathomless-firmament-v16-spark-sm121-vllm8f86f42-b12xfe06f49-fi801d57a-cu132-${DATE_TAG}}"
...
export VLLM_BUILD_VERSION="${VLLM_BUILD_VERSION:-0.11.2.dev280+fathomless.firmament.v16.vllm8f86f42.b12xfe06f49.fi801d57a.sm121.cu132.${DATE_TAG}}"
...
FLASHINFER_WHEEL_STASH=".tmp-flashinfer-wheels.disabled-ds4dspark-v16-spark-${DATE_TAG}"
...
# ---------------------------------------------------------------------------
# Post-build validation, ported from the canonical recipe (docker -> podman).
