...
# 524288 qualified 2026-08-12: checkpoint is native-1M yarn; boundary admission/
# rejection clean, needle-at-519k exact, 4/4 concurrent, KV pool 996,282 on the
# qualifying boot (per-boot UMA variance; single 524k session always fits).
...
# Model identity is owned exclusively by SERVED_MODEL_NAME. In particular,
# DeepSeek-V4-Flash is a different checkpoint and must never be advertised as
# an alias for DeepSeek-V4-Flash-0731.
if [[ " ${EXTRA_VLLM_ARGS_APPEND:-} " =~ [[:space:]]--served-model-name([=[:space:]]) ]]; then
  echo "EXTRA_VLLM_ARGS_APPEND must not override --served-model-name" >&2
  exit 2
fi
...
nccl_ch_args=()
if [[ -z "${NCCL_MAX_NCHANNELS+x}" ]]; then
  _ch=4
else
  _ch=${NCCL_MAX_NCHANNELS}
fi
if [[ -n "${_ch}" ]]; then
