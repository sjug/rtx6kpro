#!/usr/bin/env bash
# CANDIDATE (unqualified): GLM-5.2 EXL3 on Infernal Invocation r10 Spark.
# Deltas vs run-glm52-exl3-tp4-node.sh: II image default; the r33 NCCL
# LD_PRELOAD/VLLM_NCCL_SO_PATH pins are REMOVED (the II image env points at
# its own sm_121-built NCCL 2.31.2; the r33 path does not exist there and
# silently empties vLLM's allreduce backend list, breaking cross-node DCP).
set -euo pipefail
...
IMAGE=${IMAGE:-localhost/voipmonitor/vllm:infernal-invocation-r10-spark-sm121-vllmd650d5c-b12x5d648d9-fi1ac6942-cu133-torch213-20260813}
...
podman run -d \
  ...
  ${VLLM_EXL3_TRELLIS_MAX_M:+-e VLLM_EXL3_TRELLIS_MAX_M="$VLLM_EXL3_TRELLIS_MAX_M"} \
  ${VLLM_EXL3_TRELLIS_BLOCK_M:+-e VLLM_EXL3_TRELLIS_BLOCK_M="$VLLM_EXL3_TRELLIS_BLOCK_M"} \
  ${VLLM_EXL3_PREFILL_TRELLIS:+-e VLLM_EXL3_PREFILL_TRELLIS="$VLLM_EXL3_PREFILL_TRELLIS"} \
  ${VLLM_EXL3_PREFILL_CHUNK:+-e VLLM_EXL3_PREFILL_CHUNK="$VLLM_EXL3_PREFILL_CHUNK"} \
  ${VLLM_EXL3_PREFILL_CAPACITY:+-e VLLM_EXL3_PREFILL_CAPACITY="$VLLM_EXL3_PREFILL_CAPACITY"} \
