#!/usr/bin/env bash
# Matched R29 protocol, current image identity required, no custom output filenames.
set -euo pipefail
...
for host in dusty kirby; do
  ssh -n -o BatchMode=yes "$host" 'podman inspect qwen38-flash-next-nvfp4-jj-r32-tp2' > "$out/$host-benchmark-container.json"
...
HOST=http://dusty:8000 MODEL=Qwen3.8-Flash-Next-NVFP4-4p89 \
  ...
  CAMPAIGN=2026-09-jj-r32-vs-r29 VARIANT=jj-r32-aligned-sm121-tp2-mtp3 \
...
for host in dusty kirby; do
  ssh -n -o BatchMode=yes "$host" 'podman logs --timestamps qwen38-flash-next-nvfp4-jj-r32-tp2' > "$out/$host-after-grid.log" 2>&1
done
