...
# GLM-5.3-Flash NVFP4 JJ r32 Spark profile for the four-node
...
NAME=${NAME:-glm53-flash-nvfp4-jj-r32-spark-tp4}
PORT=${PORT:-8000}
# The r32 image entrypoint is the reviewed launcher (no policy block). The
# optional mount keeps the runner and launcher revisions paired for controls.
runner_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
GLM_LAUNCHER_FILE=${GLM_LAUNCHER_FILE:-${runner_dir}/launchers/serve-glm53-flash-jj-r32-spark.sh}
...
# Carry the qualified R27 aligned profile. R32 is not qualified yet.
...
if [[ -n "${GLM_LAUNCHER_FILE:-}" ]]; then
  ...
  launcher_mount=(-v "${GLM_LAUNCHER_FILE}:/usr/local/bin/serve-glm53-flash-jj-r32-spark.sh:ro")
fi
cmd=(
  podman run -d
  --name "${NAME}"
  ...
  --entrypoint /usr/local/bin/serve-glm53-flash-jj-r32-spark.sh
  "${IMAGE}"
  --nnodes "${NNODES}"
  --node-rank "${NODE_RANK}"
...
if [[ -z "${EXPECTED_IMAGE_ID}" ]]; then
  echo 'EXPECTED_IMAGE_ID is unset; pin the built r32 Spark image before launch' >&2
  exit 78
fi
...
jq -e '
  ...
  ."local-inference.b12x.spark-source-patches" == "none; the r32 B12X integration tree is applied unmodified" and
  ...
  ."local-inference.lmcache.integration.tree" == "dc1dad48e97cb849fcccf4be289e86bf8449988d" and
  ."local-inference.lmcache.package.tree" == "65712f842365a0e3944a8350e4a8c7f14bff6a11" and
  ."local-inference.lmcache.default" == "disabled"
' <<<"${labels}" >/dev/null || {
  echo "Image provenance does not match the JJ r32 Spark contract" >&2
  exit 78
}
