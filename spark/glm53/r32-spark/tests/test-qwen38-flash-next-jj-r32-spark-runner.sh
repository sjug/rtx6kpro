...
runner=${1:-"$(dirname "$0")/../run-qwen38-flash-next-jj-r32-spark-tp2-node.sh"}
...
for output in "${head_render}" "${worker_render}"; do
  ...
  require "${output}" 'serve-qwen38-flash-next-jj-r32-spark.sh'
...
require "${stop_render}" 'podman stop -t 60 qwen38-flash-next-nvfp4-jj-r32-tp2'
...
# The qualified Qwen profile is block-aligned recurrent checkpoints; the r32
# default (auto) serialized identical requests and blocked fresh arrivals.
...
stray=$(HOME="${test_home}" ROLE=head DRY_RUN=1 IMAGE="${image}" \
...
echo 'Qwen3.8 Flash Next NVFP4 JJ r32 Spark TP2 runner: PASS'
