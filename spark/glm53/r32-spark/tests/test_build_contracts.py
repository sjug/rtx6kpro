...
from prepare_sources import validate_native_digests
...
class BuildContracts(unittest.TestCase):
    ...
    def test_frozen_sources_and_paths(self):
        ...
        for name, path in [
            ...
            ("lmcache", "csrc/new.cu"),
            ("lmcache", "setup.py"),
            ("lmcache", "setup_extensions/new.py"),
            ("b12x", "b12x/new.py"),
            ("vllm", "../oops.py"),
        ]:
    ...
    def test_receipt_and_both_launcher_digests(self):
        lock = json.loads((ROOT / "source.lock.json").read_text())
        reuse = lock["native_reuse"]
        self.assertEqual(
            hashlib.sha256((ROOT / reuse["base_receipt_input"]).read_bytes()).hexdigest(),
            reuse["base_receipt_sha256"],
        )
        # A standalone build export carries the mandatory copy above. In the
        # full checkout also reject drift in its original provenance receipt.
        original = ROOT.parent / reuse["base_receipt"]
        if original.exists():
            self.assertEqual(hashlib.sha256(original.read_bytes()).hexdigest(), reuse["base_receipt_sha256"])
        for name, expected in lock["launchers"].items():
            with self.subTest(launcher=name):
                self.assertEqual(hashlib.sha256((ROOT / "launchers" / name).read_bytes()).hexdigest(), expected)
    ...
    def test_binary_digests_are_checked_against_base_evidence(self):
        previous = {"flashkda": {"spark_extension_sha256": "f" * 64}}
        receipt = {"flashkda_sha256": "f" * 64, "stable_native_sha256": "s" * 64}
        validate_native_digests(previous, receipt, "f" * 64, "s" * 64)
        for field in ("flashkda_sha256", "stable_native_sha256"):
            with self.subTest(field=field), self.assertRaises(AssertionError):
                validate_native_digests(previous, {**receipt, field: "wrong"}, "f" * 64, "s" * 64)
        with self.assertRaises(AssertionError):
            validate_native_digests({"flashkda": {"spark_extension_sha256": "wrong"}}, receipt, "f" * 64, "s" * 64)
