...
base=0b15723cb87646bb4628c5cd67aa2a7c879ac5a27105109ea5db06c30113dc0b
...
echo '16bba062c83574820d3a097414d672deef8d6079b1abe1c7f989980424654901  upstream-r32.source.lock' | sha256sum -c -
python3 tests/test_build_contracts.py
python3 tests/test_native_reuse.py
bash tests/test-glm53-flash-jj-r32-spark-tp4-runner.sh
bash tests/test-qwen38-flash-next-jj-r32-spark-runner.sh
...
glm_sha=$(sha256sum launchers/serve-glm53-flash-jj-r32-spark.sh | cut -d' ' -f1)
[[ $glm_sha == "$(value '.launchers["serve-glm53-flash-jj-r32-spark.sh"]')" ]]
version="0.26.1rc0+glm53.jj.r32.spark.vllm$(value '.vllm.package_tree' | cut -c1-7)"
label_args=()
...
podman build "${common[@]}" --target runtime --iidfile "$receipt/candidate.id" \
  ...
  --build-arg "R32_GLM_LAUNCHER_SHA256=$glm_sha" --build-arg "RECIPE_DIRTY=$recipe_dirty" \
