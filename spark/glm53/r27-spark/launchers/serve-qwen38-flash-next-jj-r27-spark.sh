...
# JJ r27 Spark profile for Qwen3.8-Flash-Next NVFP4-4p89. The host runner
# supplies the two-node topology. This deliberately preserves the qualified
# r15p Qwen execution envelope so the r27 comparison changes the source line,
...
# uses on its x86 launchers, exhausted GB10 host memory here on 2026-09-06
# (dusty unresponsive, kirby NCCL watchdog); keep the streamed loader.
load_format=${LOAD_FORMAT:-instanttensor}
# Qwen-specific: r27's default recurrent checkpoint policy (auto, resolving to
...
if [[ "${DRY_RUN:-0}" == 1 ]]; then
  ...
  printf 'Qwen3.8 Flash Next NVFP4 JJ r27 Spark launch:'
  printf ' %q' "${cmd[@]}"
  printf '\n'
  exit 0
fi
...
'; then
  printf 'JJ r27 Spark runtime source-path preflight failed; refusing launch.\n' >&2
  exit 78
fi
