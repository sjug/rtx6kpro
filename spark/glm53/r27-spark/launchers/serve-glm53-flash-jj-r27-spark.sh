...
# JJ r27 Spark launcher for GLM-5.3-Flash. The host runner owns topology and
...
if [[ "${DRY_RUN:-0}" == 1 ]]; then
  ...
  printf 'GLM-5.3-Flash JJ r27 Spark launch:'
  printf ' %q' "${cmd[@]}"
  printf '\n'
  exit 0
fi
...
'; then
  printf 'JJ r27 Spark runtime preflight failed; refusing launch.\n' >&2
  exit 78
fi
