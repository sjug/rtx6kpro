#!/usr/bin/env python3
"""Verify the JJ r27 Spark source, native, and SM121 runtime contract."""
...
def main() -> None:
    ...
    # R27 source markers: the pinned PR 674 scalar casts, the leading-instruction
    ...
    assert "tl.cast(block, tl.int64)" in checkpoint
    assert "tl.cast(slot, tl.int64)" in checkpoint
    assert (vllm_root / "tests/v1/worker/test_boundary_checkpoint_restore_scalars.py").is_file()
    renderer = (vllm_root / "vllm/renderers/online_renderer.py").read_text()
    assert "recurrent_instruction_boundary" in renderer
    draft_head = (vllm_root / "vllm/models/glm5next/nvidia/mtp_draft_head.py").read_text()
    assert "_SUPPORTED_NVFP4_CAPABILITIES = frozenset(((12, 0), (12, 1)))" in draft_head

    # B12X invariants carried from r26 plus the r27 loader sources, which stay
    ...
    assert hashlib.sha256(flashkda_object.read_bytes()).hexdigest() == args.flashkda_sha256
    cubins = subprocess.check_output(
        ["cuobjdump", "--list-elf", str(flashkda_object)], text=True
    )
    assert "sm_121" in cubins, cubins
    ...
    print("JJ r27 Spark runtime contract: PASS")
