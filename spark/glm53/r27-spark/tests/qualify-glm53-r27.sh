...
# UNCHANGED (no --recurrent-checkpoint-policy on the GLM command line).
...
for n in "${nodes[@]}"; do
  ...
  cmdline=$(ssh -n "${n}" "podman exec ${name} tr '\\0' ' ' </proc/1/cmdline; podman inspect --format '{{join .Args \" \"}}' ${name}")
  if grep -q 'recurrent-checkpoint-policy' <<<"${cmdline}"; then echo "${n}: GLM command line sets a checkpoint policy; the window requires the unchanged policy"; exit 1; fi
