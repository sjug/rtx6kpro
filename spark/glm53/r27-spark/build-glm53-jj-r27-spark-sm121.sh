...
# Build the JJ r27 GLM and Qwen runtime for SM121 from the qualified r26 Spark
# image. R27 reuses the unchanged SM121 FlashKDA object and the SM121 LMCache
# wheel, refreshes the exact vLLM and B12X source trees to the published r27
# identities, and pins vLLM PR 674 on top of the vLLM tree.
...
vllm_overlay=$(jq -er '.vllm.spark_overlay_patch' "${lock}")
vllm_pr674=$(jq -er '.vllm.pr674.patch' "${lock}")
b12x_refresh=$(jq -er '.b12x.refresh_patch' "${lock}")
b12x_paths=$(jq -er '.b12x.changed_paths' "${lock}")
for path in "${vllm_refresh}" "${vllm_paths}" "${vllm_overlay}" "${vllm_pr674}" "${b12x_refresh}" "${b12x_paths}"; do
...
verify_sha "$(jq -er '.vllm.spark_overlay_patch_sha256' "${lock}")" "${vllm_overlay}"
verify_sha "$(jq -er '.vllm.pr674.patch_sha256' "${lock}")" "${vllm_pr674}"
...
vllm_commit=$(jq -er '.vllm.r27_integration_commit' "${lock}")
vllm_tree=$(jq -er '.vllm.r27_integration_tree' "${lock}")
vllm_r27_package=$(jq -er '.vllm.r27_package_tree' "${lock}")
vllm_pr674_commit=$(jq -er '.vllm.pr674.head_commit' "${lock}")
...
b12x_r27_commit=$(jq -er '.b12x.r27_integration_commit' "${lock}")
b12x_tree=$(jq -er '.b12x.r27_integration_tree' "${lock}")
b12x_package=$(jq -er '.b12x.r27_package_tree' "${lock}")
...
test "$(jq -er '.published_r27.digest' "${lock}")" = sha256:a298fe1cd207eaf97bd2ff2686716ed25b7009c09b36650eba732a4a7dc51512
test "$(jq -er '.published_r27.source_lock_sha256' "${lock}")" = e964f1c53c693eb83c3476c89a4a40e823f6d012f5ef9588ff5ae025ab18dd91
test "${base_image_id}" = bb9cb676e464425acb945118bc541fd917fa7eb7f3024418f1733b70de31e8b8
test "${base_vllm_tree}" = d4571e5ba189fcc05534ecae88dee763ffef4e35
test "${base_vllm_package}" = 59c9787400c85d5d4547ef898c5dee41804e7088
test "${base_b12x_tree}" = c20b6aab67ed791cc226ee91de23f7c45509f436
test "${base_b12x_package}" = 00248b09689830e55d8fbce8ee630600b63ef663
test "${vllm_commit}" = 63a82f8d323e8538cbe6f88ae1812a1c01577a0f
test "${vllm_tree}" = f54cd9ca2b9434727715197d32150b75e82a9ebf
test "${vllm_r27_package}" = a336313dfd12c6b692051d1d3291712dda0c2a3d
test "${vllm_pr674_commit}" = 04813092b20f76b71159b75672a4f0d8f97c0f9e
test "${vllm_package}" = f3c3fef548738807f7d1ef8157df63815c498607
test "${vllm_spark_tree}" = 8881b7772e1644d63162ad3b7494888ef7ac54fd
...
test "${b12x_r27_commit}" = e8ad299b174f16e2e8fb5879bea272f4efbb53f2
test "${b12x_tree}" = f3cd8a9eb00d3226a1acbbed1efedf10cc1c3e71
test "${b12x_package}" = 95fdcb1cfea380480b8882fa44055cfef358ddbb
...
test "$(jq -er '.lmcache.rebuilt' "${lock}")" = false
...
if rg -n -v '\.(py|sh|cmake|md)$' "${vllm_paths}"; then
  echo 'The r27 vLLM refresh contains an unclassified path.' >&2
  exit 1
fi
if rg -n -v '\.(py|c|md|json|json\.gz|txt|toml|sh)$' "${b12x_paths}"; then
  echo 'The r27 B12X refresh contains an unclassified path.' >&2
  exit 1
fi
# The refresh patch must already carry the pinned PR 674 change.
grep -Fq 'tl.cast(block, tl.int64)' "${vllm_refresh}"
grep -Fq 'test_boundary_checkpoint_restore_scalars.py' "${vllm_paths}"

source_lock_sha256=$(sha256sum "${lock}" | cut -d' ' -f1)
recipe_sha256="$({
  sha256sum Dockerfile.glm53-jj-r27-spark-sm121 \
    ...
    "${lock}" "${vllm_refresh}" "${vllm_paths}" "${vllm_overlay}" "${vllm_pr674}" \
...
if ((worktree_dirty == 1 && allow_dirty_build == 0)); then
  echo 'Commit the r27 recipe or set ALLOW_DIRTY_BUILD=1 with an explicit dev image.' >&2
  exit 1
fi

vllm_version=${VLLM_PACKAGE_VERSION:-0.26.1rc0+glm53.jj.r27.spark.${revision}.vllm${vllm_package:0:7}.b12x${b12x_package:0:7}}
cache_fingerprint=cu133-torch213-jj-r27-sm121-vllm${vllm_package:0:10}-b12x${b12x_package:0:10}-flashkda${flashkda_commit:0:10}
image=${IMAGE:-localhost/voipmonitor/vllm:glm53-jj-r27-spark-sm121-vllm${vllm_package:0:7}-b12x${b12x_package:0:7}-lmcache${lmcache_package:0:7}-cu133-torch213-${release_date}-${revision}}
...
if [[ "${PRINT_RELEASE_CONFIG:-0}" == 1 ]]; then
  ...
  printf 'vllm_commit=%s\nvllm_tree=%s\nvllm_pr674=%s\nvllm_spark_tree=%s\nvllm_package=%s\n' "${vllm_commit}" "${vllm_tree}" "${vllm_pr674_commit}" "${vllm_spark_tree}" "${vllm_package}"
  printf 'flashkda_commit=%s\nflashkda_sha256=%s\n' "${flashkda_commit}" "${flashkda_sha}"
  printf 'b12x_r27=%s\nb12x_tree=%s\nb12x_package=%s\n' "${b12x_r27_commit}" "${b12x_tree}" "${b12x_package}"
  printf 'lmcache_r26=%s\nlmcache_tree=%s\nlmcache_rebuilt=false\nlmcache_default=disabled\n' "${lmcache_r26}" "${lmcache_tree}"
  printf 'vllm_version=%s\ncache_fingerprint=%s\nrecipe_sha256=%s\nsource_lock_sha256=%s\n' "${vllm_version}" "${cache_fingerprint}" "${recipe_sha256}" "${source_lock_sha256}"
  exit 0
fi

"${repo_root}/tests/test-glm53-flash-jj-r27-spark-tp4-runner.sh" \
  "${repo_root}/run-glm53-flash-jj-r27-spark-tp4-node.sh"
"${repo_root}/tests/test-qwen38-flash-next-jj-r27-spark-runner.sh" \
  "${repo_root}/run-qwen38-flash-next-jj-r27-spark-tp2-node.sh"
if ((worktree_dirty == 0)); then
  ...
  runner_default=$(HOME="${runner_home}" ROLE=head DRY_RUN=1 NODE_RANK=0 \
    HOST_IP=10.11.11.1 "${repo_root}/run-glm53-flash-jj-r27-spark-tp4-node.sh")
  ...
  qwen_runner_default=$(HOME="${runner_home}" ROLE=head DRY_RUN=1 NODE_RANK=0 \
    HOST_IP=10.11.1.1 "${repo_root}/run-qwen38-flash-next-jj-r27-spark-tp2-node.sh")
...
test "$(uname -m)" = aarch64 || { echo 'JJ r27 Spark builds require aarch64.' >&2; exit 78; }
command -v "${container_engine}" >/dev/null
"${container_engine}" image exists "${base_image}" || { echo "Missing qualified r26 Spark base image: ${base_image}" >&2; exit 78; }
...
"${container_engine}" build --format docker --pull=false \
  ...
  --build-arg "VLLM_R27_COMMIT=${vllm_commit}" \
  --build-arg "VLLM_R27_TREE=${vllm_tree}" \
  --build-arg "VLLM_R27_PACKAGE_TREE=${vllm_r27_package}" \
  --build-arg "VLLM_PR674_COMMIT=${vllm_pr674_commit}" \
  ...
  --build-arg "B12X_R27_COMMIT=${b12x_r27_commit}" \
  ...
  --build-arg 'RELEASE_NAME=glm53-jj-r27-spark-sm121' \
  ...
  --file Dockerfile.glm53-jj-r27-spark-sm121 \
  --tag "${image}" .
...
assert_label local-inference.scope glm53-qwen38-r27-spark-sm121-qualification
assert_label local-inference.runtime.native-artifact-base-id "${base_image_id}"
assert_label local-inference.vllm.r27.commit "${vllm_commit}"
assert_label local-inference.vllm.integration.tree "${vllm_tree}"
assert_label local-inference.vllm.r27.package.tree "${vllm_r27_package}"
assert_label local-inference.vllm.pr674.commit "${vllm_pr674_commit}"
...
assert_label local-inference.b12x.r27.commit "${b12x_r27_commit}"
...
assert_label local-inference.lmcache.rebuilt false
...
grep -Fq -- '--load-format instanttensor' <<<"${glm_launch}"
for forbidden in VLLM_GLM53_L2_PREFETCH B12X_DYNAMIC_WORK_SOURCE B12X_DYNAMIC_SPLIT_COMPUTE_MAC B12X_DYNAMIC_DIRECT_EXPERT_SCALES; do
  if grep -Fq -- "${forbidden}" <<<"${glm_launch}"; then
    echo "The GLM launcher unexpectedly sets ${forbidden}." >&2
    exit 1
  fi
done
...
if ((skip_gpu_check == 0)); then
  # In r27 importing vllm.v1.worker.gpu_worker reaches boundary_checkpoint.py,
  # which evaluates tl.constexpr at import time; without an active Triton
  # driver vLLM stubs Triton out and the import fails. The RoCE health test
  # therefore needs the GPU device even though it exercises no kernel.
  "${container_engine}" run --rm --device nvidia.com/gpu=all --ipc=host \
    -e PYTHONUNBUFFERED=1 --entrypoint /opt/venv/bin/python "${image}" -m pytest -q \
  ...
  "${container_engine}" run --rm --device nvidia.com/gpu=all --ipc=host \
    ...
    /opt/local-inference/verify_jj_r27_runtime.py \
    ...
    --flashkda-sha256 "${flashkda_sha}" \
    --source-lock-sha256 "${source_lock_sha256}"
  # Pinned PR 674: the specialized-scalar checkpoint restore regression.
  "${container_engine}" run --rm --device nvidia.com/gpu=all --ipc=host \
    -e PYTHONUNBUFFERED=1 --entrypoint /opt/venv/bin/python "${image}" -m pytest -s -vv \
    --confcutdir=/opt/jovian-judgement/vllm/tests/v1/worker \
    /opt/jovian-judgement/vllm/tests/v1/worker/test_boundary_checkpoint_restore_scalars.py
  # R27 hybrid cache regressions (endpoint checkpoints, aligned reuse, Mamba
  # resume) exercised on the SM121 runtime rather than trusted from upstream.
  "${container_engine}" run --rm --device nvidia.com/gpu=all --ipc=host \
    -e PYTHONUNBUFFERED=1 --entrypoint /opt/venv/bin/python "${image}" -m pytest -q \
    --confcutdir=/opt/jovian-judgement/vllm/tests/v1 \
    /opt/jovian-judgement/vllm/tests/v1/core/prefix_cache/test_partial_prefix_cache_hits.py \
    /opt/jovian-judgement/vllm/tests/v1/core/test_mamba_align_chunk_split.py \
    /opt/jovian-judgement/vllm/tests/v1/worker/test_mamba_prefix_state_index.py
  "${container_engine}" run --rm --device nvidia.com/gpu=all --ipc=host \
    -e PYTHONUNBUFFERED=1 --entrypoint /opt/venv/bin/python "${image}" -m pytest -s -vv \
