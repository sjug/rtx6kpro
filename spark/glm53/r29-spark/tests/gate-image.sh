...
bash tests/check-glm-launcher-identity.sh "$image" \
  launchers/serve-glm53-flash-jj-r29-spark.sh \
  "$(value '.launchers["serve-glm53-flash-jj-r29-spark.sh"]')"
...
podman run --pull=never --rm --device nvidia.com/gpu=all --ipc=host --entrypoint python "$image" \
  /opt/local-inference/verify_r29_runtime.py \
...
podman run --pull=never --rm --device nvidia.com/gpu=all --ipc=host -e PYTHONUNBUFFERED=1 \
  ...
  '
for launcher in serve-glm53-flash-jj-r29-spark.sh serve-qwen38-flash-next-jj-r29-spark.sh; do
...
output=$(podman run --pull=never --rm --entrypoint bash "$image" -c '
  set -euo pipefail
  launcher=/usr/local/bin/serve-glm53-flash-jj-r29-spark.sh
