...
runner=${1:-"$(dirname "$0")/../run-glm53-flash-jj-r29-spark-tp4-node.sh"}
...
for output in "${head_render}" "${buddy_render}" "${rocky_render}" "${lucky_render}"; do
  ...
  require "${output}" 'serve-glm53-flash-jj-r29-spark.sh'
...
launcher="$(dirname "${runner}")/launchers/serve-glm53-flash-jj-r29-spark.sh"
...
require "${stop_render}" 'podman stop -t 60 glm53-flash-nvfp4-jj-r29-spark-tp4'
...
echo 'GLM-5.3 Flash NVFP4 JJ r29 Spark TP4 runner: PASS'
