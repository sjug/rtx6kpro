...
runner=${1:-"$(dirname "$0")/../run-qwen38-flash-next-jj-r29-spark-tp2-node.sh"}
...
for output in "${head_render}" "${worker_render}"; do
  ...
  require "${output}" 'serve-qwen38-flash-next-jj-r29-spark.sh'
...
require "${stop_render}" 'podman stop -t 60 qwen38-flash-next-nvfp4-jj-r29-tp2'
...
# The qualified Qwen profile is block-aligned recurrent checkpoints; the r29
...
echo 'Qwen3.8 Flash Next NVFP4 JJ r29 Spark TP2 runner: PASS'
