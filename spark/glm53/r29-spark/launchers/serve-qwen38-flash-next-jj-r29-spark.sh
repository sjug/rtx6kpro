...
# JJ r29 Spark profile for Qwen3.8-Flash-Next NVFP4-4p89. The host runner
# supplies the two-node topology. This deliberately preserves the qualified
# R28 Qwen execution envelope so the R29 comparison changes the source line,
...
# the R27 correctness set. Both R29 profiles retain aligned pending qualification.
...
if [[ "${DRY_RUN:-0}" == 1 ]]; then
  ...
  printf 'Qwen3.8 Flash Next NVFP4 JJ r29 Spark launch:'
  printf ' %q' "${cmd[@]}"
  printf '\n'
  exit 0
fi
...
'; then
  printf 'JJ r29 Spark runtime source-path preflight failed; refusing launch.\n' >&2
  exit 78
fi
