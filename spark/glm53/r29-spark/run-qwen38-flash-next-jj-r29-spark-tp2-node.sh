...
NAME=${NAME:-qwen38-flash-next-nvfp4-jj-r29-tp2}
PORT=${PORT:-8000}
...
# Carry R28's qualified aligned profile into the R29 candidate.
# Keep the policy explicit in rank command receipts; R29 is not qualified yet.
...
cmd=(
  ...
  --entrypoint /usr/local/bin/serve-qwen38-flash-next-jj-r29-spark.sh
  "${IMAGE}"
  --nnodes "${NNODES}"
  --node-rank "${NODE_RANK}"
...
[[ -n "${EXPECTED_IMAGE_ID}" ]] || { echo 'EXPECTED_IMAGE_ID is unset; pin the built r29 image' >&2; exit 78; }
...
jq -e '
  ."local-inference.scope" == "glm53-qwen38-r29-spark-sm121-qualification" and
...
' <<<"${labels}" >/dev/null || { echo 'Image provenance does not match r29' >&2; exit 78; }
