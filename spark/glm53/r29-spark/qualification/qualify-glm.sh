#!/usr/bin/env bash
# Authorized GLM R29 aligned qualification; does not restart any service.
set -euo pipefail
...
model=GLM-5.3-Flash
name=glm53-flash-nvfp4-jj-r29-spark-tp4
...
OUT_DIR=$out BASE_URL=$base MODEL=$model python3 "$r27/tests/glm/prefix-matrix.py" r29-aligned
...
HOST=$base MODEL=$model MODEL_FAMILY=glm-5.3-flash MODEL_VARIANT=nvfp4 \
  CAMPAIGN=2026-09-jj-r29-vs-r28 VARIANT=jj-r29-aligned-sm121-tp4-dcp1-mtp3-native1m CONCURRENCY=1,2,4 \
