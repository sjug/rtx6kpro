...
def main():
    ...
    report = {"comparison": "historical single-boot R28 aligned versus R29 aligned",
              "repeatable_gain_proven": False, "grids_valid": True, "inputs": {},
              "summary": [], "cells": [], "prefill": []}
    for label, path in (("r28", args.baseline), ("r29", args.candidate)):
    ...
    for c in (1, 2, 4):
        ...
        for metric in ("server_steps_per_s", "aggregate_tps", "server_accept_len_effective"):
            ...
            row[metric] = {"r28": av, "r29": bv, "change_pct": (bv / av - 1) * 100}
        report["summary"].append(row)
    for key in sorted(a):
        report["cells"].append({"concurrency": key[0], "context_tokens": key[1],
            **{metric: {"r28": a[key][metric], "r29": b[key][metric],
    ...
    for context in sorted(baseline["prefill"], key=int):
        ...
        report["prefill"].append({"context_tokens": int(context), "r28": av, "r29": bv,
