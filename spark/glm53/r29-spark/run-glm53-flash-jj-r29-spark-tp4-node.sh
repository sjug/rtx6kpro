...
# GLM-5.3-Flash NVFP4 JJ r29 Spark profile for the four-node
...
IMAGE=${IMAGE:-localhost/voipmonitor/vllm:jj-r29-spark-sm121}
EXPECTED_IMAGE_ID=${EXPECTED_IMAGE_ID:-}
NAME=${NAME:-glm53-flash-nvfp4-jj-r29-spark-tp4}
PORT=${PORT:-8000}
# The r29 image entrypoint is the reviewed launcher (no policy block). The
# optional mount keeps the runner and launcher revisions paired for controls.
runner_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
GLM_LAUNCHER_FILE=${GLM_LAUNCHER_FILE:-${runner_dir}/launchers/serve-glm53-flash-jj-r29-spark.sh}
...
# Carry the qualified R27 aligned profile. R29 is not qualified yet.
...
if [[ -n "${GLM_LAUNCHER_FILE:-}" ]]; then
  ...
  launcher_mount=(-v "${GLM_LAUNCHER_FILE}:/usr/local/bin/serve-glm53-flash-jj-r29-spark.sh:ro")
fi
cmd=(
  podman run -d
  --name "${NAME}"
  ...
  --entrypoint /usr/local/bin/serve-glm53-flash-jj-r29-spark.sh
  "${IMAGE}"
  --nnodes "${NNODES}"
  --node-rank "${NODE_RANK}"
...
if [[ -z "${EXPECTED_IMAGE_ID}" ]]; then
  echo 'EXPECTED_IMAGE_ID is unset; pin the built r29 Spark image before launch' >&2
  exit 78
fi
...
jq -e '
  ."local-inference.vllm.integration.tree" == "27839e64744f20c16c36604e8bdfe80aea1029a0" and
  ."local-inference.vllm.package.tree" == "74b29cc58cf9f815a7133a0df4f52682eddd06f6" and
  ."local-inference.vllm.spark-overlay.tree" == "ec2025d80cdade8a1083e1ce61f0607bb227f5d1" and
  ...
  ."local-inference.b12x.spark-source-patches" == "none; the r29 B12X integration tree is applied unmodified" and
  ...
  ."local-inference.lmcache.integration.tree" == "cd8a219da05eec6c7d6c24712f9ee1a9186c3567" and
  ."local-inference.lmcache.package.tree" == "7b1043f59605ae5035f557dbf19b7382fc7eaf3f" and
  ."local-inference.lmcache.default" == "disabled"
' <<<"${labels}" >/dev/null || {
  echo "Image provenance does not match the JJ r29 Spark contract" >&2
  exit 78
}
