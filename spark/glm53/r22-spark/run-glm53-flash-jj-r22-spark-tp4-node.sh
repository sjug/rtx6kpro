...
# GLM-5.3-Flash NVFP4 JJ r22 Spark profile for the four-node
...
NAME=${NAME:-glm53-flash-nvfp4-jj-r22-spark-tp4}
PORT=${PORT:-8000}

MODEL_REPO_DIR=${MODEL_REPO_DIR:-models--local-inference-lab--GLM-5.3-Flash-NVFP4}
MODEL_REVISION=${MODEL_REVISION:-2e8b6daeb2be8716c06b54d746d12e581e551cca}
...
if [[ -z "${EXPECTED_IMAGE_ID}" ]]; then
  echo 'EXPECTED_IMAGE_ID is unset; pin the built r22 Spark image before launch' >&2
  exit 78
fi
...
jq -e '
  ...
  ."local-inference.b12x.spark-source-patches" == "aa90a277,1a7e3ec2,9ae41c5c" and
  ...
  ."local-inference.lmcache.integration.tree" == "683ab2c165a9aa0e2d1a1ab757af4a8b193688c5" and
  ."local-inference.lmcache.package.tree" == "976a97f22c0497f34db089dc5f02a713dd0b5888" and
  ."local-inference.lmcache.default" == "disabled"
' <<<"${labels}" >/dev/null || {
  echo "Image provenance does not match the JJ r22 Spark contract" >&2
  exit 78
}
