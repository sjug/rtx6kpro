...
runner=${1:-"$(dirname "$0")/../run-glm53-flash-jj-r22-spark-tp4-node.sh"}
image=localhost/voipmonitor/vllm:glm53-jj-r22-spark-sm121-vllm4fbb1c2-b12xe1edb6d-lmcache976a97f-cu133-torch213-20260904-r1
test_home=$(mktemp -d)
...
for output in "${head_render}" "${buddy_render}" "${rocky_render}" "${lucky_render}"; do
  ...
  require "${output}" '2e8b6daeb2be8716c06b54d746d12e581e551cca'
  ...
  require "${output}" 'serve-glm53-flash-jj-r22-spark.sh'
...
require "${stop_render}" 'podman stop -t 60 glm53-flash-nvfp4-jj-r22-spark-tp4'
...
echo 'GLM-5.3 Flash NVFP4 JJ r22 Spark TP4 runner: PASS'
