...
# JJ r22 Spark launcher for GLM-5.3-Flash. The host runner owns topology and
...
dflash_revision=${DFLASH_MODEL_REVISION-aea0ac8a05624512ca9e106c09c16087da998426}
...
# R22's published W4A4 target path and split target/recurrent cache geometry.
...
if [[ "${DRY_RUN:-0}" == 1 ]]; then
  printf 'GLM-5.3-Flash JJ r22 Spark launch:'
  printf ' %q' "${cmd[@]}"
  printf '\n'
  exit 0
fi
...
'; then
  printf 'JJ r22 Spark runtime preflight failed; refusing launch.\n' >&2
  exit 78
fi
