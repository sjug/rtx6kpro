...
# GLM-5.3-Flash NVFP4 JJ r26 Spark profile for the four-node
...
IMAGE=${IMAGE:-localhost/voipmonitor/vllm:glm53-jj-r26-spark-sm121-vllm59c9787-b12x00248b0-lmcachefe5442f-cu133-torch213-20260905-r1}
EXPECTED_IMAGE_ID=${EXPECTED_IMAGE_ID:-bb9cb676e464425acb945118bc541fd917fa7eb7f3024418f1733b70de31e8b8}
NAME=${NAME:-glm53-flash-nvfp4-jj-r26-spark-tp4}
PORT=${PORT:-8000}
# Ship the reviewed launcher alongside this runner. The original R26 image
# entrypoint contains the unqualified policy block removed by this control.
runner_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
GLM_LAUNCHER_FILE=${GLM_LAUNCHER_FILE:-${runner_dir}/launchers/serve-glm53-flash-jj-r26-spark.sh}
...
if [[ -n "${GLM_LAUNCHER_FILE:-}" ]]; then
  ...
  launcher_mount=(-v "${GLM_LAUNCHER_FILE}:/usr/local/bin/serve-glm53-flash-jj-r26-spark.sh:ro")
fi
cmd=(
  podman run -d
  --name "${NAME}"
  ...
  --entrypoint /usr/local/bin/serve-glm53-flash-jj-r26-spark.sh
  "${IMAGE}"
  --nnodes "${NNODES}"
  --node-rank "${NODE_RANK}"
...
if [[ -z "${EXPECTED_IMAGE_ID}" ]]; then
  echo 'EXPECTED_IMAGE_ID is unset; pin the built r26 Spark image before launch' >&2
  exit 78
fi
...
jq -e '
  ."local-inference.vllm.integration.tree" == "c861b31da5cf527d96b5772a6709450b23c93a6f" and
  ."local-inference.vllm.package.tree" == "59c9787400c85d5d4547ef898c5dee41804e7088" and
  ."local-inference.vllm.spark-overlay.tree" == "d4571e5ba189fcc05534ecae88dee763ffef4e35" and
  ."local-inference.flashkda.commit" == "3b225bf26bb8e218928a1fe14751cb48cf31d11b" and
  ."local-inference.b12x.integration.tree" == "c20b6aab67ed791cc226ee91de23f7c45509f436" and
  ."local-inference.b12x.package.tree" == "00248b09689830e55d8fbce8ee630600b63ef663" and
  ."local-inference.b12x.spark-source-patches" == "none; required RoCEnante and dense-GEMM fixes are ancestors of the r26 base" and
...
' <<<"${labels}" >/dev/null || {
  echo "Image provenance does not match the JJ r26 Spark contract" >&2
  exit 78
}
