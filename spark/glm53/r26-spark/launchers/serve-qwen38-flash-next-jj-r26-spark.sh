...
# JJ r26 Spark profile for Qwen3.8-Flash-Next NVFP4-4p89. The host runner
# supplies the two-node topology. This deliberately preserves the qualified
# r15p Qwen execution envelope so the r26 comparison changes the source line,
...
if [[ "${DRY_RUN:-0}" == 1 ]]; then
  ...
  printf 'Qwen3.8 Flash Next NVFP4 JJ r26 Spark launch:'
  printf ' %q' "${cmd[@]}"
  printf '\n'
  exit 0
fi
...
'; then
  printf 'JJ r26 Spark runtime source-path preflight failed; refusing launch.\n' >&2
  exit 78
fi
