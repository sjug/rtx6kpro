...
NAME=${NAME:-qwen38-flash-next-nvfp4-jj-r26-tp2}
PORT=${PORT:-8000}
...
[[ -n "${EXPECTED_IMAGE_ID}" ]] || { echo 'EXPECTED_IMAGE_ID is unset; pin the built r26 image' >&2; exit 78; }
...
jq -e '
  ."local-inference.scope" == "glm53-qwen38-r26-spark-sm121-qualification" and
...
' <<<"${labels}" >/dev/null || { echo 'Image provenance does not match r26' >&2; exit 78; }
