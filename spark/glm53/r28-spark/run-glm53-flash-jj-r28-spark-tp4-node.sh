...
# GLM-5.3-Flash NVFP4 JJ r28 Spark profile for the four-node
...
IMAGE=${IMAGE:-localhost/voipmonitor/vllm:jj-r28-spark-sm121}
EXPECTED_IMAGE_ID=${EXPECTED_IMAGE_ID:-}
NAME=${NAME:-glm53-flash-nvfp4-jj-r28-spark-tp4}
PORT=${PORT:-8000}
# The r28 image entrypoint is the reviewed launcher (no policy block). The
# optional mount keeps the runner and launcher revisions paired for controls.
runner_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
GLM_LAUNCHER_FILE=${GLM_LAUNCHER_FILE:-${runner_dir}/launchers/serve-glm53-flash-jj-r28-spark.sh}
...
if [[ -n "${GLM_LAUNCHER_FILE:-}" ]]; then
  ...
  launcher_mount=(-v "${GLM_LAUNCHER_FILE}:/usr/local/bin/serve-glm53-flash-jj-r28-spark.sh:ro")
fi
cmd=(
  podman run -d
  --name "${NAME}"
...
if [[ -z "${EXPECTED_IMAGE_ID}" ]]; then
  echo 'EXPECTED_IMAGE_ID is unset; pin the built r28 Spark image before launch' >&2
  exit 78
fi
...
jq -e '
  ."local-inference.vllm.integration.tree" == "a8b355eb38a0b75b97eae06738b03c65b1e9fbb5" and
  ."local-inference.vllm.package.tree" == "3db6db578029da6c4bf88c34991a69cf3571a2e5" and
  ."local-inference.vllm.spark-overlay.tree" == "4b935ffc1d9ff38196ddddbc359110ea35c0a128" and
  ...
  ."local-inference.b12x.spark-source-patches" == "none; the r28 B12X integration tree is applied unmodified" and
  ...
  ."local-inference.lmcache.integration.tree" == "247f81b6fd3b156c402a0dd31cd4a21ae8e21160" and
  ."local-inference.lmcache.package.tree" == "85622f21a2a3ae115e7a4ca41801fe221001e87f" and
  ."local-inference.lmcache.default" == "disabled"
' <<<"${labels}" >/dev/null || {
  echo "Image provenance does not match the JJ r28 Spark contract" >&2
  exit 78
}
