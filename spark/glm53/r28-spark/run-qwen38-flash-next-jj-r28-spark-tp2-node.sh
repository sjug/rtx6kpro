...
NAME=${NAME:-qwen38-flash-next-nvfp4-jj-r28-tp2}
PORT=${PORT:-8000}
...
cmd=(
  ...
  --entrypoint /usr/local/bin/serve-qwen38-flash-next-jj-r28-spark.sh
  "${IMAGE}"
  --nnodes "${NNODES}"
  --node-rank "${NODE_RANK}"
...
[[ -n "${EXPECTED_IMAGE_ID}" ]] || { echo 'EXPECTED_IMAGE_ID is unset; pin the built r28 image' >&2; exit 78; }
...
jq -e '
  ."local-inference.scope" == "glm53-qwen38-r28-spark-sm121-qualification" and
...
' <<<"${labels}" >/dev/null || { echo 'Image provenance does not match r28' >&2; exit 78; }
