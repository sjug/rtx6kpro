...
exec > >(tee "$root/cleanup.log") 2>&1
[[ ! -e $root/images-before.json ]] || { echo 'Receipt exists; refusing a duplicate run'; exit 78; }
running=$(podman ps -q)
...
podman images -a --format json > "$root/images-before.json"
podman ps -a --external --format json > "$root/containers-before.json"
podman volume ls --format json > "$root/volumes-before.json"
podman system df > "$root/storage-before.txt"
df -B1 /home/jugs > "$root/filesystem-before.txt"
