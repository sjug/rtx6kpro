...
set -euo pipefail
image=$1 version=$2 flash_sha=$3 lock_sha=$4
...
podman run --pull=never --rm --device nvidia.com/gpu=all --ipc=host --entrypoint python "$image" \
  /opt/local-inference/verify_r28_runtime.py \
  ...
  --flashkda-sha256 "$flash_sha" --source-lock-sha256 "$lock_sha"
podman run --pull=never --rm --device nvidia.com/gpu=all --ipc=host -e PYTHONUNBUFFERED=1 \
  ...
  '
for launcher in serve-glm53-flash-jj-r28-spark.sh serve-qwen38-flash-next-jj-r28-spark.sh; do
...
output=$(podman run --pull=never --rm --entrypoint bash "$image" -c '
  set -euo pipefail
  launcher=/usr/local/bin/serve-glm53-flash-jj-r28-spark.sh
