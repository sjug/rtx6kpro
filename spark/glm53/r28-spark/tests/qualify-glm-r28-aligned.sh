#!/usr/bin/env bash
# Authorized GLM R28 aligned qualification; does not restart any service.
set -euo pipefail
repo=/home/jugs/git/rtx6kpro
r28=$repo/spark/glm53/r28-spark
r27=$repo/spark/glm53/r27-spark
out=$r28/qualification/glm-20260908/aligned-mtp3
base=http://sparky:8000
model=GLM-5.3-Flash
name=glm53-flash-nvfp4-jj-r28-spark-tp4
...
OUT_DIR=$out BASE_URL=$base MODEL=$model python3 "$r27/tests/glm/prefix-matrix.py" r28-aligned
...
python3 "$r28/tests/verify-glm-native-context.py" --base-url "$base" --model "$model" \
...
HOST=$base MODEL=$model MODEL_FAMILY=glm-5.3-flash MODEL_VARIANT=nvfp4 \
  CAMPAIGN=2026-09-jj-r28-vs-r27 VARIANT=jj-r28-aligned-sm121-tp4-dcp1-mtp3-native1m CONCURRENCY=1,2,4 \
  ...
  --metadata recurrent_checkpoint_policy=aligned | tee "$out/benchmark.log"
