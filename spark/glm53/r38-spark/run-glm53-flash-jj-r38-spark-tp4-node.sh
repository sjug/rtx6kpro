...
# GLM-5.3-Flash NVFP4 JJ r38 Spark profile for the four-node
...
IMAGE=${IMAGE:-localhost/voipmonitor/vllm:jj-r38-spark-sm121}
EXPECTED_IMAGE_ID=${EXPECTED_IMAGE_ID:-}
NAME=${NAME:-glm53-flash-nvfp4-jj-r38-spark-tp4}
PORT=${PORT:-8000}
# The r38 image entrypoint is the reviewed launcher (no policy block). The
# optional mount keeps the runner and launcher revisions paired for controls.
runner_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
GLM_LAUNCHER_FILE=${GLM_LAUNCHER_FILE:-${runner_dir}/launchers/serve-glm53-flash-jj-r38-spark.sh}
...
# Carry the qualified R27 aligned profile. R38 is not qualified yet.
...
if [[ -n "${GLM_LAUNCHER_FILE:-}" ]]; then
  ...
  launcher_mount=(-v "${GLM_LAUNCHER_FILE}:/usr/local/bin/serve-glm53-flash-jj-r38-spark.sh:ro")
fi
cmd=(
  podman run -d
  --name "${NAME}"
  ...
  --entrypoint /usr/local/bin/serve-glm53-flash-jj-r38-spark.sh
  "${IMAGE}"
  --nnodes "${NNODES}"
  --node-rank "${NODE_RANK}"
...
if [[ -z "${EXPECTED_IMAGE_ID}" ]]; then
  echo 'EXPECTED_IMAGE_ID is unset; pin the built r38 Spark image before launch' >&2
  exit 78
fi
...
jq -e '
  ."local-inference.runtime.source-lock.sha256" == "9a0dc0bb8eac7d3ccf5778765146651d35982810d8fb1ed9e39832f855247be0" and
  ."local-inference.vllm.integration.tree" == "301dddf6d8fd2393aeac4d68e3a954d19911507f" and
  ."local-inference.vllm.package.tree" == "2d387573ba313b2ac667f6ea8d3f2c5b5cb36125" and
  ."local-inference.vllm.spark-overlay.tree" == "077347fdeab296404d0b0cb297d316176acc635a" and
  ."local-inference.flashkda.commit" == "3b225bf26bb8e218928a1fe14751cb48cf31d11b" and
  ."local-inference.b12x.integration.tree" == "6abad73444018f7ce1f0bdc3f0992649a4c21722" and
  ."local-inference.b12x.package.tree" == "c9ef6afbf92274069cd9f3eff9fc7dbc866d5734" and
  ."local-inference.b12x.spark-source-patches" == "none; R38 B12X integration tree applied unmodified" and
  ...
  ."local-inference.lmcache.integration.tree" == "5a88a1ea9d2627c76288d056e7193f7454669b64" and
  ."local-inference.lmcache.package.tree" == "e7c1cc7b6969d2677d7fb0c88254f9021f252020" and
  ."local-inference.lmcache.default" == "disabled"
' <<<"${labels}" >/dev/null || {
  echo "Image provenance does not match the JJ r38 Spark contract" >&2
  exit 78
}

if [[ -n "${GLM_LAUNCHER_FILE:-}" ]]; then
  [[ -f $GLM_LAUNCHER_FILE && -r $GLM_LAUNCHER_FILE && -x $GLM_LAUNCHER_FILE ]] || {
    echo 'Mounted GLM launcher must be a readable executable file' >&2; exit 78;
  }
  expected_launcher_sha=$(jq -er '."local-inference.launcher.glm.sha256"' <<<"${labels}")
  actual_launcher_sha=$(sha256sum "${GLM_LAUNCHER_FILE}" | cut -d' ' -f1)
  [[ $actual_launcher_sha == "$expected_launcher_sha" ]] || {
    echo 'Mounted GLM launcher differs from the gated image launcher' >&2; exit 78;
  }
fi
