#!/usr/bin/env python3
"""Install R38 Python and rebuilt CPU payloads with a before/after inventory."""
...
def main():
    ...
    receipt = Path("/opt/local-inference/r38-native")
    ...
    before = json.loads(Path("/opt/local-inference/r38-base-native-manifest.json").read_text())
    ...
    assert len(wheels) == 1
    source = Path("/opt/lmcache/source-r38")
    inventory = lmcache_preinstall_inventory(
        source, lock["lmcache"]["base_tree"], lock["lmcache"]["tree"], before,
        wheels[0], rebuilt_cpu=True,
    )
    Path("/opt/local-inference/r38-lmcache-preinstall-inventory.json").write_text(json.dumps(inventory, indent=2) + "\n")
    ...
    Path("/opt/local-inference/r38-native-manifest.json").write_text(json.dumps(expected, indent=2) + "\n")
    for path, expected_digest in json.loads(Path("/opt/local-inference/r38-reused-products.json").read_text()).items():
    ...
    print("R38 runtime install and classified native replacement: PASS", flush=True)
