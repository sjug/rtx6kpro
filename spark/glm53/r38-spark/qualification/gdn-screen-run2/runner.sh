...
GDN_SCREEN_ARM=${GDN_SCREEN_ARM:?set GDN_SCREEN_ARM=b12x|flashinfer}
case "$GDN_SCREEN_ARM" in b12x|flashinfer) ;; *) exit 78;; esac
NAME=${NAME:-qwen38-flash-next-r38-gdn-screen-$GDN_SCREEN_ARM}
experiment_launcher=$(cd "$(dirname "$0")" && pwd)/launcher.sh
[[ -x "$experiment_launcher" ]] || { echo 'Experiment launcher must be executable' >&2; exit 78; }
launcher_sha=$(sha256sum "$experiment_launcher" | cut -d' ' -f1)
PORT=${PORT:-8000}
...
# Carry R29's qualified aligned profile into the R38 candidate.
# Keep the policy explicit in rank command receipts; R38 is not qualified yet.
...
cmd=(
  ...
  -v "${experiment_launcher}:/usr/local/bin/serve-qwen38-flash-next-jj-r38-spark.sh:ro"
  -e QWEN_LAUNCHER_SHA256="${launcher_sha}"
  -e GDN_SCREEN_ARM="${GDN_SCREEN_ARM}"
  --entrypoint /usr/local/bin/serve-qwen38-flash-next-jj-r38-spark.sh
  "${IMAGE}"
  --nnodes "${NNODES}"
  --node-rank "${NODE_RANK}"
...
[[ -n "${EXPECTED_IMAGE_ID}" ]] || { echo 'EXPECTED_IMAGE_ID is unset; pin the built r38 image' >&2; exit 78; }
...
' <<<"${labels}" >/dev/null || { echo 'Image provenance does not match r38' >&2; exit 78; }
