...
release_label=${RELEASE_LABEL:-r38}
case "$release_label" in r32|r38) ;; *) exit 2;; esac
out="${QWEN_RECEIPT_ROOT:-$base/qwen-20260915}/$profile"
...
python3 "$r27/tests/probe-prefix-reuse.py" "${common[@]}" --label "$release_label-$profile" --receipt-file "$out/prefix-reuse.jsonl"
