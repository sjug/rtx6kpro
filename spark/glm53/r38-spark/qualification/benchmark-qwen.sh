...
profile=${1:-initial}
case "$profile" in initial|aligned-mtp3-restored) ;; *) echo 'An aligned MTP3 profile is required'; exit 2;; esac
...
for host in dusty kirby; do
  ssh -n -o BatchMode=yes "$host" 'podman inspect qwen38-flash-next-nvfp4-jj-r38-tp2' > "$out/$host-benchmark-container.json"
...
HOST=http://dusty:8000 MODEL=Qwen3.8-Flash-Next \
  ...
  CAMPAIGN=2026-09-jj-r38-vs-r32 VARIANT=jj-r38-aligned-sm121-tp2-mtp3 \
...
for host in dusty kirby; do
  ssh -n -o BatchMode=yes "$host" 'podman logs --timestamps qwen38-flash-next-nvfp4-jj-r38-tp2' > "$out/$host-after-grid.log" 2>&1
done
