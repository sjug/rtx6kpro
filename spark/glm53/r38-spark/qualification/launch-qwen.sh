#!/usr/bin/env bash
# Only dusty/kirby Qwen. Preserve R32 rollback containers and all model/JIT caches.
set -euo pipefail
...
image=${EXPECTED_IMAGE_ID:?Pin the gated R38 image ID}
[[ "$image" =~ ^[0-9a-f]{64}$ ]] || exit 2
base=$(cd "$(dirname "$0")" && pwd)
out="$base/qwen-20260915/$profile"
mkdir -p "$out"
[[ ! -e "$out/launch.log" ]] || { echo 'Receipt already exists; inspect before repeating.' >&2; exit 78; }
exec > >(tee "$out/launch.log") 2>&1
runner=/home/jugs/git/bld-jj-r38-spark/run-qwen38-flash-next-jj-r38-spark-tp2-node.sh
name=qwen38-flash-next-nvfp4-jj-r38-tp2
...
while (( $(date +%s) < deadline )); do
  ...
  if curl --fail-with-body --silent --show-error --connect-timeout 3 --max-time 60 http://dusty:8000/v1/chat/completions \
      ...
      -d '{"model":"Qwen3.8-Flash-Next","messages":[{"role":"user","content":"Calculate 17 times 23 minus 58. Reply with only the resulting integer."}],"chat_template_kwargs":{"enable_thinking":false},"temperature":0,"max_tokens":32}' \
