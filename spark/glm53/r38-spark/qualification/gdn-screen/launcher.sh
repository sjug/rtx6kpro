...
# JJ r38 Spark profile for Qwen3.8-Flash-Next NVFP4-4p89. The host runner
# supplies the two-node topology. This deliberately preserves the qualified
# R29 Qwen execution envelope so the R38 comparison changes the source line,
...
# INSTANTTENSOR_* streaming environment. fastsafetensors, which upstream R27
...
# the R27 correctness set. Both R38 profiles retain aligned pending qualification.
...
# Experiment-only backend screen; production launcher remains untouched.
unset VLLM_GDN_DECODE_KERNEL
case ${GDN_SCREEN_ARM:?set GDN_SCREEN_ARM=b12x|flashinfer} in
  b12x) gdn_args=(--gdn-decode-kernel b12x) ;;
  flashinfer) gdn_args=(--gdn-prefill-backend flashinfer) ;;
  *) echo 'Invalid GDN_SCREEN_ARM' >&2; exit 78 ;;
esac
...
if [[ "${DRY_RUN:-0}" == 1 ]]; then
  ...
  printf 'Qwen3.8 Flash Next NVFP4 JJ r38 Spark launch:'
  printf ' %q' "${cmd[@]}"
  printf '\n'
  exit 0
fi
...
assert hashlib.sha256(Path(sys.argv[1]).read_bytes()).hexdigest() == os.environ["QWEN_LAUNCHER_SHA256"]
import b12x, vllm
...
' "${BASH_SOURCE[0]}"; then
  printf 'JJ r38 Spark runtime source-path preflight failed; refusing launch.\n' >&2
  exit 78
fi
