...
archive=$root/jj-r38-spark-sm121.docker.tar
case ${1:?qwen|glm} in
  qwen) hosts=(dusty kirby); ips=(10.11.11.7 10.11.11.8);;
  kirby) hosts=(kirby); ips=(10.11.11.8);;
  glm) hosts=(sparky buddy rocky lucky); ips=(10.11.11.1 10.11.11.2 10.11.11.4 10.11.11.3);;
  *) exit 2;;
esac
cd "$root"
...
receipt=$root/qualification/transfer-$1-$(date -u +%Y%m%dT%H%M%SZ)
mkdir -p "$receipt"
...
transfer_one() {
  ...
  printf 'START %s %s\nroute=%s\n' "$host" "$(date -Is)" "$route"
  transport="ssh -o BatchMode=yes -o StrictHostKeyChecking=yes -o UserKnownHostsFile=$root/qualification/transfer-known-hosts -o HostKeyAlias=$host -o Compression=no -c aes128-gcm@openssh.com"
  read -r -a remote <<< "$transport"
  [[ $("${remote[@]}" "$ip" hostname -s) == "$host" ]] || exit 78
  "${remote[@]}" "$ip" "mkdir -p '$root'"
