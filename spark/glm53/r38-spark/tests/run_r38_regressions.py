#!/usr/bin/env python3
"""Execute the frozen R38 regressions, rejecting skips or incomplete matrices."""
...
class RequiredCases(RequiredMatrix):
    ...
    def verify(self, exit_code):
        if (exit_code != 0 or self.collected != self.expected
            ...
            raise RuntimeError(
                f"R38 matrix incomplete: expected={self.expected}, collected={self.collected}, "
                f"passed={self.passed}, exit={exit_code}, invalid={self.invalid}"
            )


def run_case(name, collect_only=False):
    ...
    if collect_only:
        ...
        print(f"R38-COLLECTION-PASS {path}: {count} collected", flush=True)
        return
    gate.verify(status)
    print(f"R38-REGRESSION-PASS {path}: {count} passed", flush=True)


def run_all(collect_only=False):
    for name in CASES:
        subprocess.run(
            [sys.executable, str(Path(__file__).resolve()), "--case", name]
            + (["--collect-only-count"] if collect_only else []),
            check=True,
...
def main():
    ...
    if args.case:
        run_case(args.case, args.collect_only_count)
    else:
        run_all(args.collect_only_count)
