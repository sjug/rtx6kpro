#!/usr/bin/env python3
"""Verify the JJ r38 Spark source, native, and SM121 runtime contract."""
...
from native_reuse import native_manifest, verify_manifest
...
def main() -> None:
    ...
    verify_manifest(
        json.loads(Path("/opt/local-inference/r38-native-manifest.json").read_text()),
        native_manifest(),
    )
    ...
    # R38 source markers: widened scalar restore, leading-instruction
    ...
    # B12X preserves the fixed dense-GEMM contract. Its newer native loader
    # remains inactive with InstantTensor and an empty VLLM_PLUGINS.
    ...
    # R29's corrected caller-owned-output schema must survive native reuse.
    ...
    print("R38 caller-owned Q output numerical regression: PASS", flush=True)
    ...
    print("JJ r38 Spark runtime contract: PASS")
