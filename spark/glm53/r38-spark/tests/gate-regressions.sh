...
gates=/opt/local-inference/r38-tests
collect=()
...
python "$gates/run_r38_regressions.py" "${collect[@]}"
required --count 2 --confcutdir=tests/v1/worker tests/v1/worker/test_async_verified_draft_counts.py
...
export PYTHONPATH=/build/r38:/opt/jovian-judgement/b12x:/opt/jovian-judgement/vllm
...
required --count 4 --confcutdir=/build/r38/inputs/upstream/tests /build/r38/inputs/upstream/tests/test_dependency_python_patches.py
required --count 24 --confcutdir="$gates" "$gates/test_sparse_mla_sm120.py" -k prefill_dsv4_dual
cd /opt/lmcache/source-r38
