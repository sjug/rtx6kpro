...
runner=${1:-"$(dirname "$0")/../run-qwen38-flash-next-jj-r38-spark-tp2-node.sh"}
image=localhost/voipmonitor/vllm:jj-r38-spark-sm121
test_cache_root=$(mktemp -d)
trap 'rm -rf "${test_cache_root}"' EXIT

render() {
  CACHE="${test_cache_root}/jit" HF_CACHE="${test_cache_root}/hf" ROLE="$1" DRY_RUN=1 IMAGE="${image}" \
...
stop_render=$(CACHE="${test_cache_root}/jit" HF_CACHE="${test_cache_root}/hf" ROLE=stop DRY_RUN=1 NODE_RANK=1 \
...
for output in "${head_render}" "${worker_render}"; do
  ...
  require "${output}" '-e SERVED_MODEL_NAME=Qwen3.8-Flash-Next -e PORT='
  reject "${output}" 'SERVED_MODEL_NAME=Qwen3.8-Flash-Next-NVFP4-4p89'
  ...
  require "${output}" 'serve-qwen38-flash-next-jj-r38-spark.sh'
...
require "${stop_render}" 'podman stop -t 60 qwen38-flash-next-nvfp4-jj-r38-tp2'

mtp0=$(CACHE="${test_cache_root}/jit" HF_CACHE="${test_cache_root}/hf" ROLE=head DRY_RUN=1 IMAGE="${image}" \
  NODE_RANK=0 HOST_IP=10.11.1.1 NUM_SPECULATIVE_TOKENS=0 "${runner}")
require "${mtp0}" 'NUM_SPECULATIVE_TOKENS=0'

# Preserve block-aligned recurrent checkpoints. The older R27 auto profile
# serialized identical requests and blocked fresh arrivals.
for output in "${head_render}" "${worker_render}" "${mtp0}"; do
  require "${output}" '--recurrent-checkpoint-policy aligned'
  reject "${output}" '--recurrent-checkpoint-policy auto'
done
stray=$(CACHE="${test_cache_root}/jit" HF_CACHE="${test_cache_root}/hf" ROLE=head DRY_RUN=1 IMAGE="${image}" \
  NODE_RANK=0 HOST_IP=10.11.1.1 EXTRA_VLLM_ARGS='--stray-control-flag' "${runner}")
reject "${stray}" '--stray-control-flag'

if CACHE="${test_cache_root}/jit" HF_CACHE="${test_cache_root}/hf" ROLE=head DRY_RUN=1 IMAGE="${image}" \
...
if [[ -e "${test_cache_root}/jit" || -e "${test_cache_root}/hf" ]]; then
  echo 'DRY_RUN created cache state' >&2
  exit 1
fi

echo 'Qwen3.8 Flash Next NVFP4 JJ r38 Spark TP2 runner: PASS'
