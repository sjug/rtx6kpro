...
from contracts import (ASYNC_COUNTS_COMMIT, FROZEN_TREES, NATIVE_INPUT_UPDATES,
                       require_native_transition, require_sm121a)
...
class Contracts(unittest.TestCase):
    def test_flashinfer_preserves_spark_build_parallelism(self):
        ...
        self.assertEqual(re.findall(r"\bFLASHINFER_NVCC_THREADS=(\d+)(?=\s|$)", env), ["1"])
    ...
    def test_inputs(self):
        ...
        entries.append(lock["b12x"]["non_python_allowlist"])
        entries.append(lock["vllm"]["native_reuse_audit"])
        entries.extend(entry["patch"] for entry in lock["vllm"]["source_patches"])
        self.assertEqual([entry["commit"] for entry in lock["vllm"]["source_patches"]],
                         [ASYNC_COUNTS_COMMIT])
        ...
        for path, digest in old["vllm"]["native_inputs"].items():
            self.assertEqual(digest, lock["vllm"]["native_inputs"][path])
        self.assertTrue({"rust", "rust-toolchain.toml", "build_rust.sh", "MANIFEST.in",
                         "tools/build_rust.py"}
                        <= lock["vllm"]["native_inputs"].keys())
        self.assertEqual(lock["vllm"]["native_inputs"].keys(),
                         lock["vllm"]["target_native_inputs"].keys())
        for path, before in lock["vllm"]["native_inputs"].items():
            require_native_transition(path, before, lock["vllm"]["target_native_inputs"][path])
    ...
    def test_native_transition_requires_exact_reviewed_bytes(self):
        for path, (before, after) in NATIVE_INPUT_UPDATES.items():
            require_native_transition(path, before, after)
            for invalid in (("0" * 40, after), (before, "0" * 40)):
                with self.assertRaises(RuntimeError):
                    require_native_transition(path, *invalid)
        require_native_transition("csrc", "same", "same")
        with self.assertRaises(RuntimeError):
            require_native_transition("csrc", "old", "new")

    def test_pr756_gpu_regression_is_required(self):
        gate = (ROOT / "tests/gate-regressions.sh").read_text()
        self.assertIn("required --count 2 --confcutdir=tests/v1/worker "
                      "tests/v1/worker/test_async_verified_draft_counts.py", gate)
