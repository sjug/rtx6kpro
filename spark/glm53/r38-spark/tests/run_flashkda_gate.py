...
directory = "/opt/local-inference/r38-native/flashkda-tests"
...
code = pytest.main(
    ...
    ] + (["--collect-only"] if args.collect_only_count else []),
    plugins=[matrix],
)
if args.collect_only_count:
    matrix.verify_collection(code, 12)
    print("FLASHKDA-R38-COLLECTION-OK collected=12", flush=True)
else:
    matrix.verify(code)
    print("FLASHKDA-R38-GATE-OK collected=12 passed=12 skipped=0", flush=True)
