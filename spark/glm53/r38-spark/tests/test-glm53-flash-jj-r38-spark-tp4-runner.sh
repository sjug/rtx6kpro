...
runner=${1:-"$(dirname "$0")/../run-glm53-flash-jj-r38-spark-tp4-node.sh"}
...
render() {
  ...
  CACHE="${test_cache_root}/jit" HF_CACHE="${test_cache_root}/hf" ROLE="${role}" DRY_RUN=1 IMAGE="${image}" \
...
stop_render=$(CACHE="${test_cache_root}/jit" HF_CACHE="${test_cache_root}/hf" ROLE=stop DRY_RUN=1 NODE_RANK=2 \
...
for output in "${head_render}" "${buddy_render}" "${rocky_render}" "${lucky_render}"; do
  ...
  require "${output}" 'serve-glm53-flash-jj-r38-spark.sh'
...
control=$(CACHE="${test_cache_root}/jit" HF_CACHE="${test_cache_root}/hf" ROLE=head DRY_RUN=1 IMAGE="${image}" \
...
auto_control=$(CACHE="${test_cache_root}/jit" HF_CACHE="${test_cache_root}/hf" ROLE=head DRY_RUN=1 IMAGE="${image}" \
...
launcher="$(dirname "${runner}")/launchers/serve-glm53-flash-jj-r38-spark.sh"
...
require "${stop_render}" 'podman stop -t 60 glm53-flash-nvfp4-jj-r38-spark-tp4'

mtp0=$(
  CACHE="${test_cache_root}/jit" HF_CACHE="${test_cache_root}/hf" ROLE=head DRY_RUN=1 IMAGE="${image}" \
...
echo 'GLM-5.3 Flash NVFP4 JJ r38 Spark TP4 runner: PASS'
