...
from run_r38_regressions import CASES, RequiredCases, run_all, run_case
...
class NativeReuse(unittest.TestCase):
    def setUp(self):
        ...
        self.files = {
            "lmcache/__init__.py": b"# R38 Python\n",
    ...
    def test_inventory_classifies_source_generated_and_native(self):
        ...
        source = {package / "code.py", package / "removed_in_r38.py"}
        ...
        self.assertEqual(result[str(package / "removed_in_r38.py")], "tracked-source")
    ...
    def test_inventory_precedes_every_reinstall(self):
        ...
        # R38 also replaces FlashInfer after the shared preinstall inventory.
    ...
    def test_all_groups_run_in_separate_processes(self):
        with patch("run_r38_regressions.subprocess.run") as run:
            run_all()
        ...
        with patch("run_r38_regressions.subprocess.run", side_effect=subprocess.CalledProcessError(1, "test")) as run:
    ...
    def test_pooled_child_sets_gpu_env_and_rejects_skips(self):
        ...
        with patch.dict(sys.modules, {"pytest": pytest}), patch("run_r38_regressions.os.chdir"), \
