...
trap 'echo "R38 image gate failed at line $LINENO: $BASH_COMMAND" >&2' ERR
...
for family in glm qwen; do
  if [[ $family == glm ]]; then launcher=serve-glm53-flash-jj-r38-spark.sh; else launcher=serve-qwen38-flash-next-jj-r38-spark.sh; fi
...
gpu=(podman run --pull=never --rm --device nvidia.com/gpu=all --ipc=host \
  ...
  -e PYTHONPATH=/build/r38:/opt/jovian-judgement/vllm:/opt/jovian-judgement/b12x)
# Some modules query CUDA at import. Collection needs CDI, but executes no tests.
"${gpu[@]}" --entrypoint bash "$image" /opt/local-inference/r38-tests/gate-regressions.sh collect
"${gpu[@]}" --entrypoint python "$image" /opt/local-inference/r38-tests/verify_dependencies.py
"${gpu[@]}" --entrypoint python "$image" /opt/local-inference/r38-tests/verify_jj_r38_runtime.py \
...
"${gpu[@]}" --entrypoint bash "$image" /opt/local-inference/r38-tests/gate-regressions.sh run
