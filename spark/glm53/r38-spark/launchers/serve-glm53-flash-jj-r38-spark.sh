...
# JJ r38 Spark launcher for GLM-5.3-Flash. The host runner owns topology and
...
if [[ "${DRY_RUN:-0}" == 1 ]]; then
  ...
  printf 'GLM-5.3-Flash JJ r38 Spark launch:'
  printf ' %q' "${cmd[@]}"
  printf '\n'
  exit 0
fi
...
import os
import hashlib
launcher = Path("/usr/local/bin/serve-glm53-flash-jj-r38-spark.sh")
...
'; then
  printf 'JJ r38 Spark runtime preflight failed; refusing launch.\n' >&2
  exit 78
fi
