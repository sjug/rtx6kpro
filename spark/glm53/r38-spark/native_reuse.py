"""Inventory ARM native payloads before replacing the LMCache wheel.

Carried from R32, with an explicit three-module CPU replacement allowance.
The actual R38 CPU/CUDA repack uses the separately digest-pinned upstream
implementation through repack_lmcache.py. All other payloads stay byte-exact.
"""
...
def lmcache_preinstall_inventory(source_root, base_tree, target_tree, native_files, wheel, *, rebuilt_cpu=False):
    ...
    mistaken for compiled extensions. Source files removed by R38 are classified
    ...
    replacements = {}
    if rebuilt_cpu:
        names = {"lmcache_native", "lmcache_fs", "lmcache_redis"}
        for path in list(preserved):
            if path.name.split(".")[0] in names:
                relative = path.relative_to(package.parent).as_posix()
                if relative not in payloads:
                    raise RuntimeError(f"Missing rebuilt CPU payload: {path}")
                replacements[str(path)] = hashlib.sha256(payloads[relative]).hexdigest()
                del preserved[path]
        if {Path(p).name.split(".")[0] for p in replacements} != names:
            raise RuntimeError("Expected exactly three known CPU extensions")
    ...
    return {
        ...
        "rebuilt_native_sha256": replacements,
    }
...
def main():
    ...
    expected = json.loads(Path("/opt/local-inference/r38-native-manifest.json").read_text())
    verify_manifest(expected, native_manifest())
    if args.mode == "verify":
        print("R38 all-native byte reuse: PASS")
        return
    ...
    print(f"R38 preserved {len(native)} native payloads: {output.name}")
