...
# JJ r37 Spark profile for Qwen3.8-Flash-Next NVFP4-4p89. The host runner
# supplies the two-node topology. This deliberately preserves the qualified
# R29 Qwen execution envelope so the R37 comparison changes the source line,
...
# the R27 correctness set. Both R37 profiles retain aligned pending qualification.
...
if [[ "${DRY_RUN:-0}" == 1 ]]; then
  ...
  printf 'Qwen3.8 Flash Next NVFP4 JJ r37 Spark launch:'
  printf ' %q' "${cmd[@]}"
  printf '\n'
  exit 0
fi
...
' "${BASH_SOURCE[0]}"; then
  printf 'JJ r37 Spark runtime source-path preflight failed; refusing launch.\n' >&2
  exit 78
fi
