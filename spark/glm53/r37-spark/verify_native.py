...
def main():
    ...
    parser.add_argument("mode", choices=["stable", "flashkda", "lmcache", "proxy", "loader"])
    ...
    if args.mode == "stable":
        ...
        if module.ABI_VERSION != 1:
            raise RuntimeError("B12X loader must implement ABI 1")
        linkage = subprocess.check_output(["ldd", module.__file__], text=True)
        require_linkage(linkage)
        if not re.search(r"liburing\.so\.\S+\s+=>\s+/", linkage):
            raise RuntimeError(f"B12X loader has no resolved liburing dependency:\n{linkage}")
        print(f"B12X loader ABI 1 and liburing linkage: PASS; {module.__file__}", flush=True)
