...
mkdir -p "$receipt"
retained_tag="localhost/voipmonitor/build-components:jj-r37-${receipt##*/}"
...
podman build --pull=never --format docker --layers --jobs=1 --tag "$retained_tag" \
    -f Dockerfile.flashinfer --iidfile "$receipt/image.id" \
    --build-arg "FLASHINFER_RECIPE_SHA256=$(sha256sum Dockerfile.flashinfer | cut -d' ' -f1)" \
    --build-arg "FLASHINFER_BUILD_SCRIPT_SHA256=$(sha256sum build-flashinfer.sh | cut -d' ' -f1)" .
