...
The actual R37 CPU/CUDA repack uses the separately digest-pinned upstream
...
def lmcache_preinstall_inventory(source_root, base_tree, target_tree, native_files, wheel, *, rebuilt_cpu=False):
    ...
    mistaken for compiled extensions. Source files removed by R37 are classified
...
def main():
    ...
    expected = json.loads(Path("/opt/local-inference/r37-native-manifest.json").read_text())
    verify_manifest(expected, native_manifest())
    if args.mode == "verify":
        print("R37 all-native byte reuse: PASS")
        return
    ...
    print(f"R37 preserved {len(native)} native payloads: {output.name}")
