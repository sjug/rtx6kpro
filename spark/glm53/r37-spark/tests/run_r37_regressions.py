#!/usr/bin/env python3
"""Execute the frozen R37 regressions, rejecting skips or incomplete matrices."""
...
class RequiredCases(RequiredMatrix):
    ...
    def verify(self, exit_code):
        if (exit_code != 0 or self.collected != self.expected
            ...
            raise RuntimeError(
                f"R37 matrix incomplete: expected={self.expected}, collected={self.collected}, "
...
def run_case(name, collect_only=False):
    ...
    if collect_only:
        ...
        print(f"R37-COLLECTION-PASS {path}: {count} collected", flush=True)
        return
    gate.verify(status)
    print(f"R37-REGRESSION-PASS {path}: {count} passed", flush=True)
