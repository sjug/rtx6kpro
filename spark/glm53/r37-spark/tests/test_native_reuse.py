...
from run_r37_regressions import CASES, RequiredCases, run_all, run_case
...
class NativeReuse(unittest.TestCase):
    def setUp(self):
        ...
        self.files = {
            "lmcache/__init__.py": b"# R37 Python\n",
    ...
    def test_inventory_classifies_source_generated_and_native(self):
        ...
        source = {package / "code.py", package / "removed_in_r37.py"}
        ...
        self.assertEqual(result[str(package / "removed_in_r37.py")], "tracked-source")
    ...
    def test_inventory_precedes_every_reinstall(self):
        ...
        # R37 also replaces FlashInfer after the shared preinstall inventory.
    ...
    def test_all_groups_run_in_separate_processes(self):
        with patch("run_r37_regressions.subprocess.run") as run:
            run_all()
        ...
        with patch("run_r37_regressions.subprocess.run", side_effect=subprocess.CalledProcessError(1, "test")) as run:
    ...
    def test_pooled_child_sets_gpu_env_and_rejects_skips(self):
        ...
        with patch.dict(sys.modules, {"pytest": pytest}), patch("run_r37_regressions.os.chdir"), \
