...
# DS4/DSpark + GLM unified v16.2 image for DGX Spark (GB10, SM121, aarch64):
# v16.1 (B12X sjug codex/fathomless-firmament-v16-integration-20260716
# @ 1bcc652 — lukealonso SM121/DGX-Spark optimization
# series, see ../v16.1/) plus FlashInfer advanced to the rebuilt DSpark
# stack on current upstream main.
#
# FlashInfer source is sjug/flashinfer branch
# codex/sm120-dspark-topk256-stack-20260716 @ 3245a18: upstream main
# (which absorbed the old stack's specdec-uniform-plan series as squashed
# PR #3871, and adds SM121 NVFP4 attention #3897, MSA for SM120/121
# #3655, CuTe-DSL JIT disk cache #3874, cutlass-dsl 4.6 migration #3922)
# topped with the re-ported TOPK=256 sparse-MLA DSV4 instantiations.
# The re-port also fixes the old pin's topk==256 prefill dispatch
# (DISPATCH_BY_NH_CM(FP8,256) -> (BF16,256), matching the 128 case).
# Verified vs the old pin (voipmonitor @ 801d57a): every non-merge stack
# commit is patch-present upstream or re-ported; the only content deltas
# are upstream refactors plus that dispatch fix.
#
# Everything else is pin-identical to v16.1: vLLM 8f86f42, B12X sjug
# 1bcc652, DeepGEMM a6b593d (+SM121 patch), NCCL dfab7c1, InstantTensor
# 85e7c5f, CUTLASS d80a4e5. The v17 CUDA13 supported-archs vLLM patch
# stays NOT applied (vLLM kernels family-generic 12.0, same as v16/v16.1).
# Historical pins remain untouched in ../v16/ and ../v16.1/.
#
# Note flashinfer-build is the earliest custom stage (feeds deepgemm ->
# b12x -> vllm), so this bump recompiles the whole stack (~3.5 h on GB10).
#
...
export IMAGE="${IMAGE:-voipmonitor/vllm:fathomless-firmament-v16p2-spark-sm121-vllm8f86f42-b12x1bcc652-fi3245a18-cu132-${DATE_TAG}}"
...
export FLASHINFER_COMMIT="${FLASHINFER_COMMIT:-3245a18ee5e4ccd5bbe37b3a56d705e82e3667fe}"
export FLASHINFER_REF="${FLASHINFER_REF:-codex/sm120-dspark-topk256-stack-20260716}"
...
export VLLM_BUILD_VERSION="${VLLM_BUILD_VERSION:-0.11.2.dev280+fathomless.firmament.v16p2.vllm8f86f42.b12x1bcc652.fi3245a18.sm121.cu132.${DATE_TAG}}"
...
FLASHINFER_WHEEL_STASH=".tmp-flashinfer-wheels.disabled-ds4dspark-v16p2-spark-${DATE_TAG}"
